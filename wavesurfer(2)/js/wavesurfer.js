...
//			this.savedVolume = 0.5
	...
	return a.create = function(b) {
		...
		getPeaks: function(a) {
			...
			for (var b = this.buffer.length / a, c = ~~ (b / 10) || 1, d = this.buffer.numberOfChannels, e = [], f = [], g = 0; g < d; g++) for (var h = e[g] = [], i = this.buffer.getChannelData(g), j = 0; j < a; j++) {
		...
		createElements: function() {
			...
			if (this.waveCc = a.getContext("2d"), this.progressWave = this.wrapper.appendChild(this.style(document.createElement("wave"), {
				...
				span.innerHTML = "00:23";
				this.progressWave.appendChild(span)
